...
function getTime(PairHourArray, PairMinArray, breakHourArray, breakMinArray) {
    ...
    for (let i = 0; i<4; i++) {
        ...
        if ( nowTime >= start && nowTime < end ) {
            ...
            timePassed.innerHTML = 'Ура, кончилось';
            timeLeft.innerHTML = '';
            line.style.width = '100%';
        }
    }

    // if (hourStart == 8 && minStart == 45) {
    //     updatePairProgress(hourStart, minStart);
    // }
}

// if (now >= 0)
// setInterval(updateBreakProgress, 1000, 18, 40, 10)
let today = new Date().getDay();
if (today != 6) {       // добавь != воскресенье
    ...
    timePassed.innerHTML = 'Завтра в шарагу(';
    timeLeft.innerHTML = '';
}
